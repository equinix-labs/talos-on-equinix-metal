...
from tasks.Controllers.ConstellationCtrl import get_constellation_clusters
...
class EquinixCtrl:
    ...
    def register_vips(self, ctx):
        ...
        """
        ppaths = ProjectPathCtrl(self.constellation, self.cluster)
        
        project_vips_file_path = ppaths.get_project_vips()
    ...
    def render_vip_addresses_file(self):
        ...
        for vip in self.cluster.vips:
            ...
            with open(self.ppaths.get_vips_file_path_by_role(vip.role), 'w') as ip_addresses_file:
                ip_addresses_file.write(data[vip.role].yaml())

    def get_vips(self, vip_role: VipRole) -> ReservedVIPs:
        with open(self.ppaths.get_vips_file_path_by_role(vip_role)) as cp_address:
